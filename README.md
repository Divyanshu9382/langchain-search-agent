...
Thought: The LLM first thinks about the user's query and decides on a plan.

Action: It chooses a tool to execute (e.g., tavily_search) and determines the input for that tool.

Observation: The agent receives the output from the tool (e.g., the search results).

Repeat: The agent takes the new information from the observation and goes back to the "Thought" step. This loop continues until the agent believes it has enough information.

Final Answer: The agent synthesizes all the information it has gathered and provides a final, comprehensive answer.
...
git clone https://github.com/Divyanshu9382/langchain-search-agent.git
